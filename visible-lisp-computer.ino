/* Visible Lisp Computer - ARM uLisp 2.8
   David Johnson-Davies - www.technoblogy.com - 14th August 2019
...
ANALOGREAD, ANALOGWRITE, DELAY, MILLIS, SLEEP, NOTE, EDIT, PPRINT, PPRINTALL, REQUIRE, LISTLIBRARY, DISPLAYFN, ENDFUNCTIONS };
...
// Flags
enum flag { RETURNFLAG, ESCAPE, EXITEDITOR, LIBRARYLOADED, DISPLAYFLAG };
volatile char Flags;
...
void initworkspace () {
  ...
  for (int i=WORKSPACESIZE-1; i>=0; i--) {
    ...
    if (tstflag(DISPLAYFLAG)) PlotObject(i, 0);
    car(obj) = NULL;
    cdr(obj) = Freelist;
    Freelist = obj;
    Freespace++;
...
object *myalloc () {
  ...
  Freespace--;
  if (tstflag(DISPLAYFLAG)) PlotObject(((int)temp - (int)Workspace)>>3, 1);
  return temp;
}
...
inline void myfree (object *obj) {
  ...
  cdr(obj) = Freelist;
  Freelist = obj;
  if (tstflag(DISPLAYFLAG)) PlotObject(((int)obj - (int)Workspace)>>3, 0);
  Freespace++;
}
...
// Winbond DataFlash support for Adafruit Express boards
...
const char string178[] PROGMEM = "display";
...
// Graphics buffer
const int GBufsize = 64*6;
uint8_t GBuf[GBufsize];

// Initialisation sequence for OLED module
int const InitLen = 11;
...
void InitDisplay () {
  Wire.beginTransmission(address);
  Wire.write(commands);
  for (uint8_t i=0; i<InitLen; i++) Wire.write(Init[i]);
...
// Plot a point; mode: 0 = black, 1 = white
void PlotObject (int i, uint8_t mode) {
  if (i > 3071) return;
  uint8_t x = i & 0x3F;
  uint8_t y = i>>6;
  uint8_t page = y>>3;
  int index = x + (page<<6);
  uint8_t bitpos = y & 0x07;
  uint8_t bits = (GBuf[index] & ~(1<<bitpos)) | (mode<<bitpos);
  GBuf[index] = bits;
  Wire.beginTransmission(address);
  Single(0x21); Single(x+32); Single(x+32);  // Column
  Single(0xB0 + page + 2);                   // Page
  Wire.write(onedata);
  Wire.write(bits);
  Wire.endTransmission();
}
...
void ClearDisplay () {
  for (int p = 0; p < GBufsize; p++) GBuf[p] = 0;
  for (int p = 0 ; p < 6; p++) {
    Wire.beginTransmission(address);
    Single(0xB0 + p + 2);       // Page
    Wire.endTransmission();
    for (int q = 0 ; q < 6; q++) {
      Wire.beginTransmission(address);
      Wire.write(data);
      for (int i = 0 ; i < 22; i++) Wire.write(0);
      Wire.endTransmission();
    }
  }
}
...
void setup () {
  ...
  ClearDisplay();
  setflag(DISPLAYFLAG);
  // Test display - should draw a diagonal line
  // for (int i=0; i<3072; i=i+65) PlotObject(i, 1);
  Serial.begin(9600);
  while (!Serial);
  initworkspace();
  initenv();
...
void repl (object *env) {
  for (;;) {
    randomSeed(micros());
    if (!tstflag(DISPLAYFLAG)) gc(NULL, env);
